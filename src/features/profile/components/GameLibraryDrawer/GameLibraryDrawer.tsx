...
export function GameLibraryDrawer() {
  ...
  const [draftStore] = useState(() => {
    ...
    const { selectedCuratedGameIds, curatedGamePreferences, customGames } =
...
function librarySignature(state: LibraryState) {
  return JSON.stringify({
    ...
    custom: [...state.customGames]
      .sort((a, b) => a.id.localeCompare(b.id))
      .map((game) => ({
        ...game,
        capabilityIds: [...game.capabilityIds].sort(),
        questOverrides: Object.entries(game.questOverrides).sort(([a], [b]) =>
          a.localeCompare(b),
        ),
      })),
  });
