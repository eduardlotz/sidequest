...
export function QuestShop() {
  ...
  const { owned, points, purchase, profile } = useQuestStore(
    useShallow((state) => ({
      owned: state.ownedPackIds,
      points: state.profile.points,
      purchase: state.purchaseQuestPack,
  ...
  return (
    <ProfilePanel
      ...
      titleId="quest-shop-title"
      description={t("ui.shop.description")}
    >
            ...
            return (
              <article key={pack.id} className={styles.pack}>
                ...
                <SolidButton
                  ...
                  disabled={
                    purchased ||
                    (points < QUEST_PACK_PRICE && !profile.debugMode)
                  }
                  aria-label={
                    purchased
                      ? t("ui.shop.ownedPack", { pack: pack.title[language] })
                      : t("ui.shop.buyPack", {
                          pack: pack.title[language],
                          price: QUEST_PACK_PRICE,
                        })
                  }
                ...
                >
                  {purchased ? (
                    ...
                    <>
                      {profile.debugMode ? 0 : QUEST_PACK_PRICE} <CoinIcon />
                    </>
                  )}
                </SolidButton>
              </article>
